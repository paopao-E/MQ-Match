...
int Match::searchMatchedEdgeFromRemainQ(int vi, int vj)
{
	...
	if (!cand.empty())
	{
		for (auto& root_n : orderTree_bgNode.at(initG[vi].label).at(initG[vj].label))
		{ 
			int u = orderTree_node[root_n].u_set[0];
			if (cand.find(u) != cand.end())
			{
				for (auto& v : cand.at(u))
				{
					...
					result += countRemainQMatch(vi, vj, v, root_n, copy_cand, copy_match, copy_usedV);
				}
			}
		}
	}
...
int Match::countRemainQMatch(int vi, int vj, int v, int tree_node, u_map cand, unordered_map<int, int> match, set<int> used_v)
{
	int result = 0;
	if (orderTree.at(initG[vi].label).at(initG[vj].label).find(tree_node) != orderTree.at(initG[vi].label).at(initG[vj].label).end())
	{
		for (auto& n : orderTree.at(initG[vi].label).at(initG[vj].label).at(tree_node))
		{
			int u = orderTree_node[n].u_set[0];
			if (cand.find(u) != cand.end())
			{
				for (auto& v : cand.at(u))
				{
